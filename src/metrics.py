...
class Metrics(torch.nn.Module):
    ...
    def update_metrics(self, y_pred: Tensor, y_true: Tensor,
                       weights=Optional[Tensor], metrics: List[str] = None):
        """
        Args:
            y_pred:
            y_true:
        ...
        y_pred_act = activation(y_pred, loss_type=self.loss_type)  # run activations if y_pred is logits

        if metrics is None:
            metrics = self.metrics.keys()
        else:
            metrics = [name for name in metrics if name in self.metrics]
